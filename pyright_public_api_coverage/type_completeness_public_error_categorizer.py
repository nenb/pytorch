...
class PyrightErrorAnalyzer:
    ...
    def extract_errors_from_symbols(self, symbols: List[Dict[str, Any]]) -> List[str]:
        """Extract error messages from symbol diagnostics (severity 'error' only, exported symbols only)"""
        errors = []
        for symbol in symbols:
            # Only analyze exported symbols
            if symbol.get('isExported', False) and 'diagnostics' in symbol:
