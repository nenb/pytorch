...
def find_and_count_unknown_return_types(json_file):
    ...
    # Regex to capture X from the multi-line message:
    # "Return type is partially unknown\n  Return type is "X""
    pattern = re.compile(r'Return type is partially unknown\n\s+Return type is "([^"]+)"')
    ...
    for symbol in symbols:
        for diagnostic in symbol.get('diagnostics', []):
            ...
            if match:
                # group(1) is X (the return type)
                return_type = match.group(1)
                type_counts[return_type] += 1
...
if __name__ == "__main__":
    ...
    output_file = 'results/unknown_return_types_by_count.txt'

    total_count, counts = find_and_count_unknown_return_types(input_json)

    print(f"Total count of issues with 'Return type is partially unknown': {total_count}")
    print("-" * 30)

    if counts:
        # The most_common() method sorts the items by count in descending order.
        # Each item is a tuple: (return_type, count)
        sorted_items = counts.most_common()

        with open(output_file, 'w', encoding='utf-8') as f:
            for return_type, count in sorted_items:
                f.write(f'Return Type: "{return_type}", Count: {count}\n')

        print(f"Successfully saved sorted results to '{output_file}'")
    else:
        print("No issues with 'Return type is partially unknown' were found.")